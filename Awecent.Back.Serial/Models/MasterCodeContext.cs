    ...
    public class MasterCodeContext
    {
        ...
        public MasterCode CreateMasterCode(MasterCode model, string userName)
        {
            using (MySqlConnection con = new MySqlConnection(ItemConnection))
            {
                try
                {
                    MySqlCommand cmd = new MySqlCommand("awe_genMasterCode", con);
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    cmd.Parameters.Add(new MySqlParameter("_GameID", model.GameID));
                    cmd.Parameters.Add(new MySqlParameter("_PromotionName", model.GameID));
                    cmd.Parameters.Add(new MySqlParameter("_PromotionDetail", model.GameID));
                    cmd.Parameters.Add(new MySqlParameter("_SerailFlag", model.GameID));
                    //_StartDate,_EndDate,_ItemID,_CreateUser,_GenerateType,_PrefixCode
                    cmd.Parameters.Add(new MySqlParameter("_StartDate", model.StartAt));
                    cmd.Parameters.Add(new MySqlParameter("_EndDate", model.EndAt));
                    ...
                    string code = cmd.Parameters["_ReturnMsg"].Value.ToString();
                    if (string.IsNullOrEmpty(code))
                    {
                        model.Result = false; model.Message = "Create Master Code fail.";
                    }
                    else {
                        model.Result = true; model.Code = code; model.Message = "Create Master Code success.";
                    }
                    return model;
                }
                catch (Exception ex)
